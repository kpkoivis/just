...
/*
    ...
    public void testMovies() throws Exception {
        ...
        List<Reservation> movies = new ArrayList<>((Collection<Reservation>) res.getModelAndView().getModel().get("movies"));
        ...
        movies = new ArrayList<>((Collection<Reservation>) res.getModelAndView().getModel().get("movies"));
        assertTrue("Kun yksi elokuva lisätään, tulee elokuvien määrän kasvaa vain yhdellä.", movieCount + 1 == movies.size());
        boolean found = false;
        Long id = -1L;
        for (Reservation movie : movies) {
